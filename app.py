...
# 1. 앱 페이지 설정 (명칭 변경 및 아이콘 유지)
st.set_page_config(page_title="김앤리 보고서(Kim & Lee Report) 콘텐츠 생성기", page_icon="🫁", layout="wide")
...
Scientific Accuracy:
- Reflect accurate anatomical and physiological knowledge of the {organ}.
...
They act as an expert researcher couple, inspecting the {organ} with tiny diagnostic devices.
...
Medical integrity:
- Keep the anatomy of the {organ} educational and scientifically plausible.
...
오늘은 {organ}의 핵심 구조를 쉽고 정확하게 살펴볼게요.
...
김앤리 보고서였습니다."""
    ...
    hashtags = f"#김앤리보고서 #{organ} #건강정보 #인체해부 #의학상식 #shorts"
    description = f"""파스텔 블루/핑크 스웨터를 입은 흰 고양이 커플과 함께,
3D 미니어처 클레이 디오라마로 {organ}의 구조와 건강 포인트를 쉽게 풀어드립니다.

✅ 오늘의 핵심: {organ}의 구조, 기능, 생활 속 관리 팁
✅ 형식: 토이 월드 + 강한 틸트 시프트 + ASMR 질감 연출

{hashtags}"""
    pinned_comment = f"여러분은 {organ} 건강을 위해 오늘 어떤 습관을 실천하셨나요? 궁금한 점은 댓글로 남겨주시면 김앤리 보고서가 다음 영상에서 다뤄볼게요! 🐾"
    ...
    st.info("장기 이름을 입력하면 '김앤리 보고서' 전용 콘텐츠 팩이 생성됩니다.")
